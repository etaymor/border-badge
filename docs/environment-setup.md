...
PLACES_MAX_CONCURRENT_REQUESTS=5  # Optional: max concurrent Google Places calls per suggest-places request (1-50)
PLACES_MAX_CONCURRENT_REQUESTS_PROCESS=15  # Optional: per-process ceiling across all suggest-places requests (1-100; raise in step with VISION_MAX_CONCURRENT_REQUESTS)
SUGGEST_PLACES_CLUSTER_BUDGET_PER_MINUTE=400  # Optional: rolling per-minute cluster budget per caller for /photos/suggest-places (1-100000)
SUGGEST_PLACES_VISION_IMAGE_BUDGET_PER_MINUTE=600  # Optional: rolling per-minute vision-image budget per caller for /photos/suggest-places (1-100000)
...
VISION_MAX_CONCURRENT_REQUESTS=5  # Optional: max concurrent vision calls per suggest-places request (1-50; raise PLACES_MAX_CONCURRENT_REQUESTS_PROCESS in step with this)
QUIZ_CLASSIFICATION_BUDGET_PER_QUIZ=300  # Optional: hard cap on vision-classified images per draft quiz (1-500). A ceiling, not expected spend: creation stops as soon as the game is full (~50-70 on a healthy library). Lowering it makes "Not Enough Photos Yet" declines more likely.
QUIZ_CLASSIFICATION_DAILY_CAP=50000  # Optional: global daily classification circuit breaker (0-1000000); 0 is an emergency stop that disables the eligibility endpoint
