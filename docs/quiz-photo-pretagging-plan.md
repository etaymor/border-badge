...
2. **Seed from persisted verdicts:** stored `eligible=1` (valid `classifier_version`) go straight into `session.eligible` with stored `landscape`; stored `eligible=0` go into `classifiedIds` so no pass re-draws them. Only **fresh** seeds (not in the KTD12 used-asset ledger) are offered to the pick ledger; used-but-eligible photos are held in a reserve that is offered only once the fresh library is exhausted (nothing left to classify) or the game would otherwise fall below `QUIZ_MIN_PHOTOS`. If the fresh seeds alone reach `QUIZ_MAX_PHOTOS`, skip the hunt entirely → repeat creation is upload-bound. (2026-08-22 fix: seeding used to offer used photos too, merely ordered last, so once fewer than ten fresh verdicts were cached the previous game's photos were locked in, the hunt never ran, and every game repeated the same ten. Regression tests: `quizCreation.test.ts` "repeat creations never reuse photos".)
