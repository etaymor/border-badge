-- Atomic entry creation with entry limit enforcement
-- Fixes TOCTOU race condition where concurrent requests could bypass free tier entry limits.
-- Uses advisory lock on trip_id to ensure count check and insert are atomic.
