...
async function processImages() {
  ...
  const files = fs.readdirSync(INPUT_DIR)
    .filter(f => f.endsWith('.png') && !f.includes('_alt'));
...
function generateTypeScriptMapping(processedImages) {
  ...
  const imports = sortedImages
    .map(img => `  ${img.code}: require('../../assets/country-images/processed/${img.code}.webp'),`)
    .join('\n');
...
`;
  ...
  console.log(`   Contains ${sortedImages.length} country image imports\n`);
}
