...
async function processImages() {
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.log(`✅ ${baseName} → ${code}.webp (${inputSizeKB}KB → ${outputSizeKB}KB, -${reduction}%)`);
    } catch (err) {
  ...
  const totalReduction = totalInputSize > 0 ? ((1 - totalOutputSize / totalInputSize) * 100).toFixed(1) : 0;
...
function generateTypeScriptMapping(processedImages) {
  const outputPath = path.join(__dirname, '../src/assets/stampImages.ts');

  // Sort by code for cleaner output
  const sortedImages = [...processedImages].sort((a, b) => a.code.localeCompare(b.code));

  const imports = sortedImages
    .map(img => `  ${img.code}: require('../../assets/stamps/processed/${img.code}.webp'),`)
    .join('\n');
...
`;
  ...
  console.log(`   Contains ${sortedImages.length} stamp image imports\n`);
}
